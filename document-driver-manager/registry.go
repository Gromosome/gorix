...
func Register(adapter Adapter) {
	if adapter == nil {
		panic("gorix document-driver-manager: adapter cannot be nil")
	}
	...
	if name == "" {
		panic("gorix document-driver-manager: adapter name is required")
	}
	...
	if _, exists := adapters.values[name]; exists {
		panic(
			"gorix document-driver-manager: adapter already registered: " +
				name,
		)
	}
...
func Lookup(name string) (Adapter, error) {
	...
	if !exists {
		return nil, fmt.Errorf(
			"gorix document-driver-manager: driver %q is not registered; blank-import its wrapper module",
			name,
		)
	}
